...
public interface PartyInfoRepository extends JpaRepository<PartyInfo, Long>{
    ...
    /* native query를 사용하는 경우 sort 불가*/
    // @Query(value ="SELECT p.* FROM party_info_table p WHERE p.status in ('0','1','3') AND p.user_id = :user_id UNION SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id WHERE p.status in ('0','1','3') AND c.user_id = :user_id2 ORDER BY :search_condition DESC", nativeQuery = true)
    ...
    @Query(value ="SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id AND c.user_id =:user_id WHERE p.status in(0,1,3) AND c.carpooling_status in ('WAITING','ACCEPT','FORMED') ORDER BY start_date DESC", nativeQuery = true)
    List<PartyInfo> findMyNowListByStartDateDesc(@Param("user_id") String user_id);
    @Query(value ="SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id AND c.user_id =:user_id WHERE p.status in(0,1,3) AND c.carpooling_status in ('WAITING','ACCEPT','FORMED') ORDER BY review_average_score DESC", nativeQuery = true)
    List<PartyInfo> findMyNowListByReviewAverageScoreDesc(@Param("user_id") String user_id);
    @Query(value ="SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id AND c.user_id =:user_id WHERE p.status in(0,1,3) AND c.carpooling_status in ('WAITING','ACCEPT','FORMED')", nativeQuery = true)
    List<PartyInfo> findMyNowListNoCondition(@Param("user_id") String user_id);

    @Query(value ="SELECT p.* FROM party_info_table p WHERE p.status in ('0','1','3') AND p.start_date  LIKE  CONCAT(:times,'%') AND p.user_id = :user_id UNION SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id WHERE p.status in ('0','1','3') AND p.start_date  LIKE  CONCAT(:times,'%') AND c.user_id = :user_id2 ORDER BY start_date DESC", nativeQuery = true)
    ...
    @Query(value ="SELECT p.* FROM party_info_table p WHERE p.status in ('0','1','3') AND p.department  LIKE  CONCAT(:place,'%') AND p.user_id = :user_id UNION SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id WHERE p.status in ('0','1','3') AND p.department  LIKE  CONCAT(:place,'%') AND c.user_id = :user_id2 ORDER BY start_date DESC", nativeQuery = true)
    ...
    @Query(value ="SELECT p.* FROM party_info_table p WHERE p.status in ('0','1','3') AND p.start_date  LIKE  CONCAT(:times,'%') AND p.user_id = :user_id UNION SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id WHERE p.status in ('0','1','3') AND p.start_date  LIKE  CONCAT(:times,'%') AND c.user_id = :user_id2 ORDER BY review_average_score DESC", nativeQuery = true)
    ...
    @Query(value ="SELECT p.* FROM party_info_table p WHERE p.status in ('0','1','3') AND p.start_date  LIKE  CONCAT(:times,'%') AND p.user_id = :user_id UNION SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id WHERE p.status in ('0','1','3') AND p.start_date  LIKE  CONCAT(:times,'%') AND c.user_id = :user_id2 ORDER BY id DESC", nativeQuery = true)
    ...
    /* native query를 사용하는 경우 sort 불가*/
    // @Query(value ="SELECT p.* FROM party_info_table p WHERE p.status in ('4') AND p.user_id = :user_id UNION SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id WHERE p.status in ('4') AND c.user_id = :user_id2 ORDER BY :search_condition DESC", nativeQuery = true)
    ...
    @Query(value ="SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id AND c.user_id =:user_id WHERE p.status in(4) AND c.carpooling_status in ('ACCEPT','FORMED') ORDER BY start_date DESC", nativeQuery = true)
    List<PartyInfo> findMyPastListByStartDateDesc(@Param("user_id") String user_id);
    @Query(value ="SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id AND c.user_id =:user_id WHERE p.status in(4) AND c.carpooling_status in ('ACCEPT','FORMED') ORDER BY distance ASC", nativeQuery = true)
    List<PartyInfo> findMyPastListByDistanceDesc(@Param("user_id") String user_id);
    @Query(value ="SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id AND c.user_id =:user_id WHERE p.status in(4) AND c.carpooling_status in ('ACCEPT','FORMED') ORDER BY review_average_score DESC", nativeQuery = true)
    List<PartyInfo> findMyPastListByReviewAverageScoreDesc(@Param("user_id") String user_id);
    @Query(value ="SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id AND c.user_id =:user_id WHERE p.status in(4) AND c.carpooling_status in ('ACCEPT','FORMED') ORDER BY id DESC", nativeQuery = true)
    List<PartyInfo> findMyPastListNoCondition(@Param("user_id") String user_id);

    @Query(value ="SELECT p.* FROM party_info_table p WHERE p.status in ('4') AND p.start_date  LIKE  CONCAT(:times,'%') AND p.user_id = :user_id UNION SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id WHERE p.status in ('4') AND p.start_date  LIKE  CONCAT(:times,'%') AND c.user_id = :user_id2 ORDER BY start_date DESC", nativeQuery = true)
    ...
    @Query(value ="SELECT p.* FROM party_info_table p WHERE p.status in ('4') AND p.start_date  LIKE  CONCAT(:times,'%') AND p.user_id = :user_id UNION SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id WHERE p.status in ('4') AND p.start_date  LIKE  CONCAT(:times,'%') AND c.user_id = :user_id2 ORDER BY review_average_score DESC", nativeQuery = true)
    ...
    @Query(value ="SELECT p.* FROM party_info_table p WHERE p.status in ('4') AND p.start_date  LIKE  CONCAT(:times,'%') AND p.user_id = :user_id UNION SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id WHERE p.status in ('4') AND p.start_date  LIKE  CONCAT(:times,'%') AND c.user_id = :user_id2 ORDER BY id DESC", nativeQuery = true)
...
/*
    ...
    //Carpooler로 참여한
    @Query(value ="SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id WHERE c.user_id = :user_id ORDER BY p.start_date DESC;", nativeQuery = true)
    List<PartyInfo> findByCarPoolerUserIdOrderByMoveInfoStartDateDesc(@Param("user_id") String user_id);
    @Query(value ="SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id WHERE c.user_id = :user_id ORDER BY p.distance DESC", nativeQuery = true)
    List<PartyInfo> findByCarPoolerUserIdOrderByMoveInfoDistanceDesc(@Param("user_id") String user_id);
    @Query(value ="SELECT p.* FROM party_info_table p LEFT OUTER JOIN party_info_car_poolers c ON p.id = c.party_info_id WHERE c.user_id = :user_id ORDER BY p.review_average_score DESC", nativeQuery = true)
